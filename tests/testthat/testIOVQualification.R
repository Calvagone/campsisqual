...
test_that("ADVAN4 TRANS4 model with IOV on ALAG1 works as expected", {
  ...
  for (engine in TEST_ENGINES) {
    qualNocb <- qualify_model(ctlPath=ctlPath, modelName=modelName, dataset=dataset, dest=engine,
                             variables="CP", settings=Settings(NOCB(TRUE)), skipNM=SKIP_NONMEM_PREPARATION)
    qualLocf <- qualify_model(ctlPath=ctlPath, modelName=modelName, dataset=dataset, dest=engine,
                             variables="CP", settings=Settings(NOCB(FALSE)), skipNM=SKIP_NONMEM_PREPARATION)
...
test_that("ADVAN4 TRANS4 model with IOV on KA works as expected", {
  ...
  for (engine in TEST_ENGINES) {
    qualNocb <- qualify_model(ctlPath=ctlPath, modelName=modelName, dataset=dataset, dest=engine,
                             variables="CP", settings=Settings(NOCB(TRUE, "IOV_KA")), skipNM=SKIP_NONMEM_PREPARATION)
    qualLocf <- qualify_model(ctlPath=ctlPath, modelName=modelName, dataset=dataset, dest=engine,
                             variables="CP", settings=Settings(NOCB(FALSE, "IOV_KA")), skipNM=SKIP_NONMEM_PREPARATION)
    expect_true(qualNocb %>% passed())
    expect_true(qualLocf %>% passed())
  }
})
...
test_that("ADVAN4 TRANS4 model with IOV on CL works as expected", {
  ...
  for (engine in TEST_ENGINES) {
    qualNocb <- qualify_model(ctlPath=ctlPath, modelName=modelName, dataset=dataset, dest=engine,
                             variables="CP", seed=2, settings=Settings(NOCB(TRUE, "IOV_CL")), skipNM=SKIP_NONMEM_PREPARATION)
    qualLocf <- qualify_model(ctlPath=ctlPath, modelName=modelName, dataset=dataset, dest=engine,
                             variables="CP", seed=2, settings=Settings(NOCB(FALSE, "IOV_CL")), skipNM=SKIP_NONMEM_PREPARATION)
    expect_true(qualNocb %>% passed())
    expect_true(qualLocf %>% passed())
  }
})
