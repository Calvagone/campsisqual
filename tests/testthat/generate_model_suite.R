...
generateTmddSuite <- function() {
  ...
  tmddParameters <- Parameters() %>%
    ...
    add(Theta(
      name = "KINT",
      value = 1,
      label = 'Internalization/Degradation rate of drug-target complex',
      unit = '1/d'
    )) %>% # 1/day
...
generatePKForSimulation <- function(pk) {
  ...
  # Export dataset
  table <- final_dataset %>%
    export(dest = "mrgsolve", model = campsis, seed = 1, settings = Settings(NOCB(TRUE, "TDOS"))) %>%
