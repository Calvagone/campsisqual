...
if (is.null(original_model) || original_model=='') {
  summaryTable <- NULL
} else {
  summaryTable <- c('Original model:'=original_model)
}
summaryTable <- summaryTable %>% append(
  c('Simulation engine:'=qual_summary@dest,
    'Relative tolerance:'=qual_summary@tolerance,
    'Variable(s) compared:'=paste(qual_summary@variables, collapse=', '),
...
if (qualOK) {cat(sprintf('Status: \\textcolor{teal}{\\textbf{SUCCESSFUL}} on %s', qualificationTimeString))} else {cat(sprintf('\\textbf{Status:} \\textcolor{red}{\\textbf{FAIL}} on %s\n', qualificationTimeString))}
```

```{r}
if (!(is.null(notes) || notes=='')) {
  cat(sprintf('\\textbf{Additional notes:} %s', notes))
}
cat('\\newpage\n')
```
...
for (index in seq_along(qual_summary@ids)) {
  ...
  for (variable in qual_summary@variables) {
    ...
    if (condition) {
      plot <- qual_summary %>% get_plot(id, variable)
      cat('\\newpage\n')
